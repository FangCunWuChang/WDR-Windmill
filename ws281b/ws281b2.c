 ...
 */
static struct WS2_Port_Config WS2_port_config_list[WS2_Port_COUNT] = {
...
void WS2_control_init() {
    /** Get max LED size */
    int maxSize = 0;
    for (int i = 0; i < WS2_Port_COUNT; i++) {
...
void WS2_control_send(enum WS2_Port_ID port) {
    ...
    /** Send data */
    WS_Write_RGB_LED((const void*)WS2_port_buffer, arrowsSize + surroundingSize, handle, channel);
}
